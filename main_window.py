...
from workers import (
    ...
    apply_settings_task, find_disk_files_task, delete_disk_files_task,
    find_config_files_task, replace_config_files_task
)
...
from optimizations.ai_optimizer import get_ai_optimizer, is_ai_optimization_available
...
class NumericTableWidgetItem(QTableWidgetItem):
    """
    Custom table widget item for numeric sorting.
    
    Enables proper numerical sorting instead of lexicographic sorting
    for table columns containing numeric data.
    """
    
    def __lt__(self, other: QTableWidgetItem) -> bool:
        """Compare items numerically if possible, fallback to string comparison."""
        try:
...
class MainWindow(QMainWindow, MainWindowOptimizationMixin):
    ...
    def _on_status_updated(self, category: str, status: str, level: str):
        ...
        # Update summary in traditional status bar
        if hasattr(self, 'summary_status_label'):
            if category == 'instances':
                self.summary_status_label.setText(f"📱 Instances: {status}")
            elif category == 'ai_tracker':
                self.summary_status_label.setText(f"🤖 AI: {status}")
            elif category == 'memory':
                self.summary_status_label.setText(f"🧠 Memory: {status}")
    
    def _on_progress_updated(self, value: int, description: str):
        """Handle progress updates from status component"""
        # Can add progress indication to main status bar if needed
        if hasattr(self, 'summary_status_label') and value > 0:
            self.summary_status_label.setText(f"🔄 {description} ({value}%)")
    ...
    def _create_dashboard_page(self):
        ...
        # Connect signals for standard dashboard only - DISABLED (AI Tracker thay thế)
        # self.refresh_btn.clicked.connect(self.refresh_instances)  # DISABLED
        # self.btn_auto_refresh.toggled.connect(self._toggle_auto_refresh)  # DISABLED
        ...
        self.btn_auto_refresh.setToolTip(f"Tự động làm mới mỗi {self.auto_refresh_interval}s")
        
        # 🤖 AI CONTROL BUTTON - Control panel for AI tracker
        self.btn_ai_control = QPushButton("🤖 AI Settings")
        self.btn_ai_control.setToolTip("Open AI Tracker Control Panel")
        self.btn_ai_control.clicked.connect(self.show_ai_control_panel)
        ...
        filter_bar.addWidget(self.btn_ai_control)  # 🤖 AI Control Panel access
        left_layout.addLayout(filter_bar)
    ...
    def _create_cleanup_page(self):
        ...
        group = QGroupBox("Dọn dẹp bộ nhớ đệm (ota.vdi)")
        group_layout = QVBoxLayout(group)

        info_label = QLabel(
            "Chức năng này sẽ tìm và xóa các tệp <b>ota.vdi</b> trong thư mục chứa máy ảo của MuMu Player "
        ...
        # 🚀 MODERN DISK MANAGEMENT BUTTONS
        self.btn_find_disk_files = ModernButton("🔍 Tìm tệp ota.vdi", "primary", "md")
        self.btn_delete_disk_files = ModernButton("🗑️ Xóa các tệp đã tìm thấy", "danger", "md")
        self.btn_delete_disk_files.setEnabled(False)
        # Note: Color is automatically handled by ModernButton danger variant
        action_layout.addWidget(self.btn_find_disk_files)
        action_layout.addWidget(self.btn_delete_disk_files)
        group_layout.addLayout(action_layout)

        self.found_files_display = QTextEdit()
        self.found_files_display.setReadOnly(True)
        self.found_files_display.setPlaceholderText("Kết quả tìm kiếm sẽ được hiển thị ở đây...")
        group_layout.addWidget(self.found_files_display)
    ...
    def _create_config_page(self):
        ...
        source_group = QGroupBox("1. Chọn file Config nguồn")
        source_layout = QHBoxLayout(source_group)
        self.config_path_edit = QLineEdit()
        self.config_path_edit.setPlaceholderText("Đường dẫn đến file customer_config.json mẫu")
        browse_config_btn = QPushButton("Duyệt...")
        browse_config_btn.clicked.connect(self._browse_config_file)
        source_layout.addWidget(self.config_path_edit)
        source_layout.addWidget(browse_config_btn)
        layout.addWidget(source_group)

        action_group = QGroupBox("2. Tìm và Thực hiện thay thế")
        action_layout = QVBoxLayout(action_group)

        action_buttons_layout = QHBoxLayout()
        self.btn_find_config_files = QPushButton("🔍 Tìm tất cả file Config")
        self.btn_replace_config_files = QPushButton("🔄 Thay thế tất cả")
        self.btn_replace_config_files.setEnabled(False)
        self.btn_replace_config_files.setStyleSheet("background-color: #c0392b;")
        action_buttons_layout.addWidget(self.btn_find_config_files)
        action_buttons_layout.addWidget(self.btn_replace_config_files)
        action_layout.addLayout(action_buttons_layout)

        self.found_configs_display = QTextEdit()
        self.found_configs_display.setReadOnly(True)
        self.found_configs_display.setPlaceholderText("Danh sách các file customer_config.json tìm thấy sẽ hiển thị ở đây...")
        action_layout.addWidget(self.found_configs_display)

        layout.addWidget(action_group)
        return widget

    def _create_automation_page(self):
        """Tạo trang Tự động hóa với thiết kế Monokai cổ điển"""
        automation_page = MonokaiAutomationPage(self)
        return automation_page

    def _create_status_bar(self):
        """
        Create status bar sử dụng modular component architecture
        
        Phase 2: Replace legacy status bar với StatusComponent
        """
        
        # 🚀 PHASE 2: Use modular status component for enhanced status display
        if OPTIMIZATION_AVAILABLE:
            try:
                ...
                # Create status widget as a dock or side panel instead of traditional status bar
                status_widget = self.status_component.create_status_panel()
                
                # For now, add to main layout. Later can be made into a dock widget
                # Create a minimal status bar for compatibility
                self.status_bar = QStatusBar()
                self.setStatusBar(self.status_bar)
                
                # Add a summary status to the traditional status bar
                self.summary_status_label = QLabel("🚀 Modular Status: Ready")
                self.status_bar.addPermanentWidget(self.summary_status_label)
                
                # Store status widget for potential dock widget usage
                self.modular_status_widget = status_widget
                
                # Extract compatibility labels
                self._assign_status_component_labels()
                
                print("🚀 Phase 2: Modular status component created successfully!")
                return
                
            except Exception as e:
                print(f"⚠️ Status component creation failed: {e}")
                print("🔄 Falling back to legacy status bar")
        ...
        # Thêm label hiển thị trạng thái auto-refresh
        ...
        # 🚀 MODERN GLOBAL PROGRESS BAR - Enhanced visual feedback
    ...
    def _handle_sidebar_click(self, index):
        # ✅ LAZY LOADING: Load page on demand
        if not self.loaded_pages.get(index, False):
            self._load_page(index)
        
        self.content_stack.setCurrentIndex(index)

    def _load_page(self, index):
        """Tải nội dung của một trang một cách lười biếng."""
        if self.loaded_pages.get(index, False):
            return  # Đã tải rồi

        print(f"🔄 Loading page {index}...")

        # Xác định hàm tạo trang dựa trên index
        page_creators = {
            1: self._create_apps_page,
            2: self._create_tools_page,
            3: self._create_scripting_page,
            4: self._create_cleanup_page,
            5: self._create_config_page,
            6: self._create_automation_page,
        }
        
        creator = page_creators.get(index)
        if creator:
            try:
                print(f"📄 Creating page widget for index {index}...")
                # Tạo widget mới
                page_widget = creator() 
                print(f"✅ Page widget created successfully for index {index}")
                
                # Xóa placeholder cũ và thêm widget mới
                ...
                # Kết nối signals cho trang mới
                self._connect_page_signals(index)
                ...
                self.log_message(f"✅ Lazy Loading: Tải thành công trang {page_name}", LogLevel.INFO, "UI")
                print(f"✅ Page {index} ({page_name}) loaded successfully!")
            ...
            print(f"❌ No creator found for page index {index}")
    ...
    def _connect_page_signals(self, page_index):
        ...
        elif page_index == 4:  # Cleanup page
            if hasattr(self, 'btn_find_disk_files'):
                self.btn_find_disk_files.clicked.connect(self._start_find_disk_files)
                self.btn_delete_disk_files.clicked.connect(self._start_delete_disk_files)
        elif page_index == 5:  # Config page
            if hasattr(self, 'btn_find_config_files'):
                self.btn_find_config_files.clicked.connect(self._start_find_config_files)
                self.btn_replace_config_files.clicked.connect(self._start_replace_config_files)
            if hasattr(self, 'config_path_edit'):
                self.config_path_edit.textChanged.connect(self.update_ui_states)
                if hasattr(self, 'config_path_edit'):
                    self.config_path_edit.textChanged.connect(self.update_ui_states)
    ...
    def update_ui_states(self):
        ...
        if hasattr(self, 'btn_find_disk_files') and self.btn_find_disk_files is not None:
            self.btn_find_disk_files.setEnabled(not is_busy)
        if hasattr(self, 'btn_delete_disk_files') and self.btn_delete_disk_files is not None:
            self.btn_delete_disk_files.setEnabled(not is_busy and bool(self.found_ota_files))

        if hasattr(self, 'btn_find_config_files') and self.btn_find_config_files is not None:
            self.btn_find_config_files.setEnabled(not is_busy)
        
        # Config replace button with complex condition check
        if hasattr(self, 'btn_replace_config_files') and self.btn_replace_config_files is not None:
            can_replace = not is_busy and bool(self.found_config_files) and bool(getattr(self, 'config_path_edit', None) and self.config_path_edit.text().strip())
            self.btn_replace_config_files.setEnabled(can_replace)
    ...
    def _on_user_activity(self) -> None:
        ...
        # 🧠 Record user activity for AI learning
        if hasattr(self, 'ai_optimizer') and self.ai_optimizer:
            try:
                # Determine activity type from sender
                sender = self.sender()
                if sender:
                    activity_type = "table_interaction"
                    target = sender.__class__.__name__
                    context = {'widget_type': target}
                    
                    self.ai_optimizer.record_user_action(activity_type, target, context)
            except Exception as e:
                pass  # Silent fail for AI tracking
    ...
    def refresh_instances(self) -> None:
        ...
        # 🧠 Record refresh action for AI learning
        if hasattr(self, 'ai_optimizer') and self.ai_optimizer:
            try:
                self.ai_optimizer.record_user_action('refresh', 'refresh_button', {
                    'timestamp': time.time(),
                    'manual_refresh': True
                })
            except Exception:
    ...
    def _start_auto_refresh(self) -> None:
        """
        Start intelligent auto-refresh system.
        
        Features:
        - User activity detection to prevent interruptions
        - Smart delays based on user interaction patterns
        - State preservation during refresh operations
        - Configurable refresh intervals with minimum thresholds
        
        The system uses QTimer.singleShot for non-blocking operation
        and automatically adapts to user behavior patterns.
        """
        if self.auto_refresh_interval > 0:
            ...
            self.log_message(
                f"Tự động làm mới đã bật (mỗi {self.auto_refresh_interval}s)", 
                LogLevel.INFO, 
                "Auto Refresh", 
                {'interval': self.auto_refresh_interval}
            )
            
            # Start smart auto-refresh with initial delay (minimum 5s)
            initial_delay = max(self.auto_refresh_interval * 1000, 5000)
            QTimer.singleShot(initial_delay, self._auto_refresh_instances)
    ...
    def _update_auto_refresh_status(self) -> None:
        ...
        if hasattr(self, 'auto_refresh_status_label'):
            if self.auto_refresh_enabled:
                self.auto_refresh_status_label.setText(
                    f"🔄 Tự động làm mới: {self.auto_refresh_interval}s"
                )
                self.auto_refresh_status_label.setStyleSheet("color: green;")
            else:
                self.auto_refresh_status_label.setText("⏹️ Tự động làm mới: Tắt")
                self.auto_refresh_status_label.setStyleSheet("color: gray;")
    ...
    def _auto_refresh_instances(self) -> None:
        ...
        if (self.worker and self.worker.isRunning()) or \
           (self.refresh_worker and self.refresh_worker.isRunning()):
            # Delay auto refresh if workers are running (5s delay)
            QTimer.singleShot(5000, self._auto_refresh_instances)
            return
        ...
        if time_since_activity < self.user_interaction_delay:
            # User recently active, calculate remaining delay time
            remaining_time = (self.user_interaction_delay - time_since_activity) * 1000
            QTimer.singleShot(int(remaining_time + 1000), self._auto_refresh_instances)
            return
        ...
        if has_focus or has_selection or mouse_over_table:
            # User actively interacting with table, delay refresh (3s delay)
            QTimer.singleShot(3000, self._auto_refresh_instances)
            return
        
        # Layer 5: Check context menu state
        if hasattr(self, '_context_menu_open') and self._context_menu_open:
            # Context menu open, short delay (1s)
            QTimer.singleShot(1000, self._auto_refresh_instances)
            return
        
        # All checks passed - safe to refresh
        self._silent_refresh_instances()
        
        # Schedule next auto refresh cycle if still enabled
        if self.auto_refresh_enabled and self.auto_refresh_interval > 0:
            next_interval = max(self.auto_refresh_interval * 1000, 5000)  # Minimum 5s
            QTimer.singleShot(next_interval, self._auto_refresh_instances)

    def _silent_refresh_instances(self) -> None:
        """
        Perform silent background refresh with complete state preservation.
        
        This method implements the non-disruptive refresh functionality
        that maintains table state, selections, scroll position, and focus
        while updating data in the background.
        
        Features:
        - Complete table state preservation
        ...
        # Preserve complete table state before refresh
        self._save_table_state()

        # Try cache first for instant silent update
        cached_data = self.smart_cache.get("instances_list", "instance_dynamic")
        if cached_data and cached_data.get("success"):
            # Silent cache hit - no logging, preserve state
            self._on_instances_loaded_silent_preserve(cached_data)
            return

        # Clear failed indices for fresh attempt (no logging for auto-refresh)
        self.failed_indices.clear()

        def refresh_task(worker: GenericWorker, manager: MumuManager, params: dict) -> dict:
            """Silent background refresh task."""
            worker.started.emit("Làm mới tự động...")  # Silent progress message
            success, data = manager.get_all_info()
            worker.progress.emit(100)
            result = {"success": success, "data": data}
        ...
        # Initialize worker for silent refresh
        self.refresh_worker = GenericWorker(refresh_task, self.mumu_manager, {})
        self.refresh_worker.task_result.connect(self._on_instances_loaded_silent_preserve)
        self.refresh_worker.finished.connect(self._on_refresh_finished)
    ...
    def _toggle_auto_refresh(self) -> None:
        """
        Toggle auto-refresh system on/off.
        
        Provides user control over the auto-refresh system,
        allowing them to disable it when needed and re-enable
        with previous settings intact.
        """
        if self.auto_refresh_enabled:
    ...
    def _apply_filter(self) -> None:
        ...
        """
        # 🧠 Record search action for AI learning
        if hasattr(self, 'ai_optimizer') and self.ai_optimizer:
            try:
                keyword = self.search_edit.text().strip()
                self.ai_optimizer.record_user_action('search', 'search_box', {
                    'query': keyword,
                    'timestamp': time.time()
                })
            except Exception:
    ...
    def _apply_filter_with_database(self) -> None:
        ...
        try:
            ...
            if hasattr(self, 'instances_model') and self.instances_model is not None:
                self.instances_model.set_rows(filtered_items)
    ...
    def _control_selected_instances(self, action: str) -> None:
        """
        Control selected instances with batch processing + Ultra-Fast Database + AI optimization.
        ...
        - AI-powered predictive optimization and user behavior tracking
        ...
        # ⚡ Track AI action for predictive optimization
        if hasattr(self, 'ai_optimizer') and self.ai_optimizer:
            try:
                self.ai_optimizer.track_user_action({
                    'type': 'bulk_control',
                    'action': action,
    ...
    def update_button_icons(self):
        # Sidebar buttons - check if they exist
        if hasattr(self, 'sidebar_buttons') and self.sidebar_buttons:
            for key, icon_name in [("dashboard", "dashboard"), ("apps", "apps"), ("adb", "adb"), 
                                   ("script", "script"), ("cleanup", "cleanup"), ("config", "config"), 
                                   ("automation", "automation")]:
                if key in self.sidebar_buttons and self.sidebar_buttons[key] is not None:
                    self.sidebar_buttons[key].setIcon(get_icon(icon_name))
    ...
    def _update_selection_info(self) -> None:
        ...
        self.selection_label.setText(f"Đã chọn: {count}")
    ...
    def _start_find_disk_files(self) -> None:
        """
        Start disk file search operation.
        
        Features:
        - Configurable VMS path with fallback options
        - Directory validation with user-guided recovery
        - Settings persistence for path preferences
        - Error handling and user guidance
        
        Searches for disk files in the MuMu Player VMS directory
        and provides options for path selection if not found.
        """
        # Get VMS path from settings with fallback
        default_path = r"C:\Program Files\Netease\MuMuPlayerGlobal-12.0\vms"
        vms_path = self.settings.value("cleanup/vms_path", default_path)

        # Validate VMS directory existence
        if not os.path.isdir(vms_path):
            reply = QMessageBox.question(
                self, 
                "Không tìm thấy thư mục",
                f"Không tìm thấy thư mục máy ảo tại:\n{vms_path}\n\n"
                f"Bạn có muốn tự chọn thư mục 'vms' không?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            ...
            if reply == QMessageBox.StandardButton.Yes:
                chosen_dir = QFileDialog.getExistingDirectory(
                    self, 
                    "Chọn thư mục 'vms' của MuMu Player"
                )
                if chosen_dir:
                    vms_path = chosen_dir
                    self.settings.setValue("cleanup/vms_path", vms_path)
                else:
                    return
            else:
                return

        # Execute disk file search with validated path
        params = {
            'vms_path': vms_path,
            'type': 'ota.vdi',
            'exclude_dir': 'MuMuPlayerGlobal-12.0-base'
        }
        
        worker = GenericWorker(find_disk_files_task, self.mumu_manager, params)
        self._start_worker(worker)

    def _on_files_found(self, result: dict) -> None:
        """
        Handle disk file search results.
        
        Args:
            result: Dictionary containing found files list
        
        Features:
        - File list processing and storage
        - UI update with search results
        - User feedback about search outcomes
        - State management for subsequent operations
        """
        found_files = result.get('files', [])
        self.found_ota_files = found_files
        self.found_files_display.clear()
        
        if found_files:
            self.found_files_display.setText("\n".join(found_files))
            self.log_message(f"🔎 Tìm thấy {len(found_files)} tệp ota.vdi.")
        else:
            self.found_files_display.setPlaceholderText("Không tìm thấy tệp ota.vdi nào.")
            self.log_message("✅ Không tìm thấy tệp ota.vdi nào.")
        
        self.update_ui_states()

    def _start_delete_disk_files(self) -> None:
        """
        Start disk file deletion operation with confirmation.
        
        Features:
        - File existence validation
        - User confirmation dialog with safety measures
        - Irreversible operation warning
        - Worker-based deletion for non-blocking UI
        
        Provides a safe deletion process with multiple confirmation
        steps to prevent accidental data loss.
        """
        # Validate files to delete
        if not self.found_ota_files:
            QMessageBox.information(self, "Thông báo", "Không có tệp nào để xóa.")
            return
        ...
        reply = QMessageBox.question(
            self, 
            'Xác nhận Xóa',
            f"Bạn có chắc chắn muốn xóa vĩnh viễn {len(self.found_ota_files)} tệp đã tìm thấy không? "
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # Execute deletion with worker
            params = {'files_to_delete': self.found_ota_files}
            worker = GenericWorker(delete_disk_files_task, self.mumu_manager, params)
            worker.finished.connect(self._on_delete_finished)
            self._start_worker(worker)

    def _on_delete_finished(self, msg: str) -> None:
        """
        Handle disk file deletion completion.
        
        Args:
            msg: Completion message from deletion worker
        
        Features:
        - File list cleanup
        - UI state reset
        - Display restoration to initial state
        - Status update for user feedback
        """
        # Clear file references and UI
        self.found_ota_files = []
        self.found_files_display.clear()
        self.found_files_display.setPlaceholderText("Kết quả tìm kiếm sẽ được hiển thị ở đây...")
        self.update_ui_states()
    ...
    def _browse_config_file(self) -> None:
        ...
        Opens file dialog for selecting JSON configuration files
        and updates the configuration path in the UI.
        """
    ...
    def _start_find_config_files(self) -> None:
        """
        Start configuration file search operation.
        
        Features:
        - VMS path validation with error handling
        - Automatic path resolution from settings
        - User guidance for path configuration
        - Worker-based search for non-blocking UI
        
        Searches for customer_config.json files in the VMS directory
        structure for configuration management operations.
        """
        # Get and validate VMS path
        default_path = r"C:\Program Files\Netease\MuMuPlayerGlobal-12.0\vms"
        vms_path = self.settings.value("cleanup/vms_path", default_path)
        ...
        if not os.path.isdir(vms_path):
            QMessageBox.critical(
                ...
                f"Vui lòng kiểm tra lại cài đặt hoặc chọn thủ công trong tab Dọn dẹp Disk."
            )
            return

        # Execute config file search
        params = {'vms_path': vms_path, 'type': 'customer_config.json'}
        worker = GenericWorker(find_config_files_task, self.mumu_manager, params)
        self._start_worker(worker)

    def _on_config_files_found(self, result: dict) -> None:
        """
        Handle configuration file search results.
        
        Args:
            result: Dictionary containing found config files list
        
        Features:
        - Config file list processing and storage
        - UI update with search results
        - User feedback about search outcomes
        - State management for subsequent operations
        """
        self.found_config_files = result.get('files', [])
        self.found_configs_display.clear()
        
        if self.found_config_files:
            self.found_configs_display.setText("\n".join(self.found_config_files))
            self.log_message(f"🔎 Tìm thấy {len(self.found_config_files)} file customer_config.json.")
        else:
    ...
    def _start_replace_config_files(self) -> None:
        """
        Start configuration file replacement operation.
        
        Features:
        - Source file validation
        - Target file existence verification
        - User confirmation for file replacement
        - Worker-based replacement for non-blocking UI
        
        Replaces found customer_config.json files with a selected
        source configuration file across multiple instances.
        """
        # Validate source file
        source_file = self.config_path_edit.text().strip()
        if not source_file or not os.path.isfile(source_file):
            QMessageBox.warning(self, "Lỗi", "Vui lòng chọn một file config nguồn hợp lệ.")
            return

        if not self.found_config_files:
            QMessageBox.information(self, "Thông báo", "Không có file đích nào để thay thế. Vui lòng bấm 'Tìm' trước.")
            return

        reply = QMessageBox.question(self, 'Xác nhận Thay thế',
            f"Bạn có chắc chắn muốn ghi đè nội dung của {len(self.found_config_files)} file config không? "
            "Hành động này KHÔNG THỂ hoàn tác.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)

        if reply == QMessageBox.StandardButton.Yes:
            params = {'source_file': source_file, 'target_files': self.found_config_files}
    ...
    def _find_row_by_index(self, index: int) -> int:
        for row in range(self.table.rowCount()):
            item = self.table.item(row, TableColumn.NAME)
    ...
    def _update_table_row_deprecated(self, row: int, idx: int, info: dict):
        def get_or_create_item(r, c, item_class=QTableWidgetItem):
        ...
        if self.table.cellWidget(row, TableColumn.CHECKBOX) is None:
            cb = QCheckBox()
            cell_widget = QWidget()
            ...
            self.table.setCellWidget(row, TableColumn.CHECKBOX, cell_widget)

        # STT column now shows VM index instead of row number
        stt_item = get_or_create_item(row, TableColumn.STT, NumericTableWidgetItem)
        stt_item.setText(str(idx) if idx is not None else "N/A")
        stt_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)

        # NAME column shows the actual VM name
        name_item = get_or_create_item(row, TableColumn.NAME)
        name_item.setText(info.get("name", "N/A"))
        name_item.setData(Qt.ItemDataRole.UserRole, idx)

        status_item = get_or_create_item(row, TableColumn.STATUS)
        ui_status = self.instance_ui_states.get(idx)
        if ui_status:
            status_item.setData(Qt.ItemDataRole.UserRole, ui_status)
        else:
            status_item.setData(Qt.ItemDataRole.UserRole, info.get("is_process_started", False))

        adb_item = get_or_create_item(row, TableColumn.ADB)
        adb_item.setText(str(info.get("adb_port", "—")))

        disk_bytes = info.get("disk_size_bytes", 0)
        disk_text = "N/A"
        if disk_bytes > 0:
            disk_gb = disk_bytes / (1024**3)
            disk_text = f"{disk_gb:.2f} GB" if disk_gb >= 1 else f"{disk_bytes / (1024**2):.2f} MB"

        disk_item = get_or_create_item(row, TableColumn.DISK_USAGE)
        disk_item.setText(disk_text)
        disk_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)

        get_or_create_item(row, TableColumn.SPACER)

        error_color = QColor("#5a2929") if idx in self.failed_indices else None
        for col in range(self.table.columnCount()):
            item = self.table.item(row, col)
            if item:
                item.setBackground(error_color or self.palette().color(QPalette.ColorRole.Base))
    ...
    def _load_instance_data(self):
        ...
        try:
            self.refresh_instances()
            self.log_message("✅ Instance data loading started", LogLevel.INFO, "Startup")
        except Exception as e:
            self.log_message(f"❌ Instance data failed: {e}", LogLevel.ERROR, "Startup")
    
    def _load_auto_refresh(self):
        """Load auto refresh system"""
        try:
            if self.auto_refresh_enabled and self.auto_refresh_interval > 0:
                self._start_auto_refresh()
            self._update_auto_refresh_status()
            self.log_message("✅ Auto refresh system started", LogLevel.INFO, "Startup")
        except Exception as e:
            self.log_message(f"❌ Auto refresh failed: {e}", LogLevel.ERROR, "Startup")
    ...
    def _finish_startup(self):
        ...
        # 🧠 Initialize AI-Powered Optimization System
        self._setup_ai_optimization()
    ...
    def _setup_ai_optimization(self):
        """🧠 Setup AI-powered optimization system"""
        try:
            if hasattr(self, 'ai_optimizer') and self.ai_optimizer:
                # Connect AI signals to UI
                self.ai_optimizer.prediction_ready.connect(self._on_ai_predictions)
                self.ai_optimizer.optimization_applied.connect(self._on_ai_optimization_applied)
                self.ai_optimizer.learning_progress.connect(self._on_ai_learning_progress)
                
                # Start AI optimization with 15-second prediction intervals
                self.ai_optimizer.start_ai_optimization(15000)
                
                # Record initial startup action for learning
                self.ai_optimizer.record_user_action('startup', 'main_window', {
                    'timestamp': time.time(),
                    'app_version': APP_VERSION
                })
                
                # Get AI system insights
                ai_insights = self.ai_optimizer.get_ai_insights()
                self.log_message(
                    f"🧠 AI Optimization initialized: {ai_insights['ai_status']}", 
                    LogLevel.SUCCESS, 
                    "AI", 
                    ai_insights
                )
            ...
            self.log_message(f"❌ AI optimization setup failed: {e}", LogLevel.ERROR, "AI")
    
    def _on_ai_predictions(self, predictions):
        """Handle AI predictions ready signal"""
        try:
            if predictions:
                high_confidence_predictions = [p for p in predictions if p.confidence > 0.6]
                if high_confidence_predictions:
                    pred_info = ", ".join([f"{p.action_type}({p.confidence:.0%})" 
                                         for p in high_confidence_predictions])
                    self.log_message(
                        f"🧠 AI Predicted: {pred_info}", 
                        LogLevel.INFO, 
                        "AI Predictions"
                    )
        except Exception as e:
            print(f"❌ AI prediction handling failed: {e}")
    
    def _on_ai_optimization_applied(self, stats):
        """Handle AI optimization applied signal"""
        try:
            if stats.get('cache_hit_rate', 0) > 0:
                self.log_message(
                    f"⚡ AI Cache hit rate: {stats['cache_hit_rate']:.1f}%", 
                    LogLevel.INFO, 
                    "AI Performance",
                    stats
                )
        except Exception as e:
            print(f"❌ AI optimization stats handling failed: {e}")
    
    def _on_ai_learning_progress(self, message):
        """Handle AI learning progress updates"""
        self.log_message(message, LogLevel.INFO, "AI Learning")
    ...
    def _async_load_instance_data(self):
        """Async load instance data"""
        try:
            self.refresh_instances()
            return "Instance Data loaded"
        except Exception as e:
            return f"Instance Data failed: {e}"
    
    def _async_load_auto_refresh(self):
        """Async load auto refresh system"""
        try:
            if self.auto_refresh_enabled and self.auto_refresh_interval > 0:
                self._start_auto_refresh()
            self._update_auto_refresh_status()
            ...
            return f"Auto Refresh failed: {e}"
    ...
    def _on_ai_instance_status_changed(self, instance_id: int, status: str, data: dict):
        """Handle AI instance status changes (Performance optimized)"""
        try:
            # Silent operation để giảm console spam
            if hasattr(self.global_ai_tracker, 'debug_mode') and self.global_ai_tracker.debug_mode:
                print(f"🤖 AI Status Update: Instance {instance_id} -> {status}")
            
            # Update table if available (optimized với batch updates)
            if hasattr(self, 'table') and self.table:
                # Defer table update để avoid blocking UI
                self.global_ai_tracker.update_instance_in_table(self.table, instance_id, status, data)
            
            # Cache update với error handling
            try:
                cache_key = f"instance_{instance_id}_ai_status"
                cache_data = {'status': status, 'data': {k: v for k, v in data.items() if k != 'last_update'}}  # Remove datetime để avoid JSON error
                self.smart_cache.set(cache_key, cache_data)
            except Exception:
                pass  # Silent cache errors
            
            # Minimal logging để avoid performance impact
            if hasattr(self, 'log_message'):
                try:
                    self.log_message(f"🤖 Instance {instance_id}: {status}", LogLevel.INFO, "AI")
                except Exception:
                    pass
    ...
    def _on_ai_instances_updated(self, instances: dict):
        """Handle AI instances update (Performance optimized)"""
        try:
            active_count = len(instances)
            
            # Minimal logging để giảm spam
            if hasattr(self.global_ai_tracker, 'debug_mode') and self.global_ai_tracker.debug_mode:
                print(f"🤖 AI Update: {active_count} instances tracked")
            
            # Update status bar với throttling
            if hasattr(self, 'status_bar') and active_count > 0:
                try:
                    ai_status = f"🤖 {active_count} instances"
                    # Throttled status update
                    if not hasattr(self, '_last_ai_status') or self._last_ai_status != ai_status:
                        self._last_ai_status = ai_status
                        self.status_bar.showMessage(f"Ready | {ai_status}")
                except Exception:
                    pass
            
            # Lightweight cache update
            try:
                self.smart_cache.set("ai_tracked_count", active_count)
            except Exception:
                pass
            ...
            # Silent error handling
            if hasattr(self.global_ai_tracker, 'debug_mode') and self.global_ai_tracker.debug_mode:
                print(f"❌ AI Update Handler Error: {e}")
    
    def show_ai_control_panel(self):
        """Show AI Tracker control panel"""
        try:
            if not self.ai_control_panel:
                from ai_tracker_control import AITrackerControlPanel
                self.ai_control_panel = AITrackerControlPanel(self)
                
                # Connect signals
                self.ai_control_panel.interval_changed.connect(self.global_ai_tracker.set_scan_interval)
                self.ai_control_panel.debug_mode_changed.connect(self.global_ai_tracker.set_debug_mode)
                self.ai_control_panel.tracker_toggled.connect(self._toggle_ai_tracker)
            
            self.ai_control_panel.show()
            self.ai_control_panel.raise_()
            
        except Exception as e:
            print(f"❌ Error showing AI control panel: {e}")
    
    def _toggle_ai_tracker(self, enabled: bool):
        """Toggle AI tracker on/off"""
        try:
            if enabled:
                self.global_ai_tracker.start_tracking()
            else:
                self.global_ai_tracker.stop_tracking()
        except Exception as e:
            print(f"❌ Error toggling AI tracker: {e}")
            
            # Update AI status label
            if hasattr(self, 'ai_tracker_status'):
                if active_count > 0:
                    self.ai_tracker_status.setText(f"🤖 AI: {active_count} instances running")
                    self.ai_tracker_status.setStyleSheet("color: #A6E22E; font-weight: bold;")
                else:
                    self.ai_tracker_status.setText("🤖 AI: No instances running")
                    self.ai_tracker_status.setStyleSheet("color: #F92672; font-weight: bold;")
            
            # Update status bar with AI info
            if hasattr(self, 'status_bar'):
                ai_status = f"🤖 AI: {active_count} instances tracked"
                # Try to update status bar (if status method exists)
                try:
                    self.status_bar.showMessage(f"Ready | {ai_status}")
                except:
                    pass
            
            # Cache the instances data
            self.smart_cache.set("ai_tracked_instances", instances)
            
        except Exception as e:
            print(f"❌ Error handling AI instances update: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("✨ Phase 4.1: AI-Powered Optimization - ACTIVE")  
